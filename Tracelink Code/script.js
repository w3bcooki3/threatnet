...
function showSection(sectionName) {
    ...
    // Close Blue Team Playbook modals gracefully
    ...
    // Close TraceLink modals gracefully
    if (window.traceLinkManager) {
        ...
        if (addNodeModal && addNodeModal.style.display === 'flex') {
            window.traceLinkManager.closeAddNodeModal();
        }
...
// Hide all sections
    ...
    if (targetSection) {
        ...
        if (sectionName === 'investigation-notes') {
            ...
            // Ensure tracelinkPlaceholder is correctly selected
            ...
            if (!tracelinkPlaceholder.dataset.loaded) { // Only load HTML once
                fetch('tracelink.html')
                    ...
                    .then(html => {
                        ...
                        // Wait for multiVaultManagerReady event before initializing TraceLinkManager
                        if (typeof cytoscape !== 'undefined' && typeof window.multiVaultManager !== 'undefined' && typeof window.multiVaultManager.getVaults === 'function') {
                            ...
                            // Set up listener for multiVaultManagerReady if it's not ready yet
...
function initializeApp() {
    ...
    if (typeof CaseStudiesManager !== 'undefined') {
        // window.caseStudiesManager is already created in case-studies.js's DOMContentLoaded
        // We just need to ensure the section is properly activated when selected.
        // For initial load, if dashboard is default, nothing special is needed here.
        // If you want case studies to render immediately on load, then call its render method:
        // window.caseStudiesManager.filterCaseStudies(); // This will ensure they are rendered
    }
}
...
// Make functions globally available for TraceLink interaction
// IMPORTANT: This multiVaultManager is crucial for TraceLink to get vault data
window.multiVaultManager = {
    getVaults: function() {
        // Return the global 'vaults' array.
        // Assuming 'vaults' is declared at the top level of script.js
        return vaults;
    },
    getVaultById: function(vaultId) {
        // Find a specific vault by its ID
        return vaults.find(v => v.id === vaultId);
    },
    getEntryById: function(vaultId, entryId) {
        // Find a specific entry within a specific vault
        const vault = vaults.find(v => v.id === vaultId);
        return vault ? vault.entries.find(e => e.id === entryId) : null;
    }
};
