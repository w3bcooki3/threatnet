...
class TraceLinkManager {
    ...
    init() {
        ...
        this.importModal = document.getElementById('tracelink-import-modal'); // New import modal
    ...
    _setupEventListeners() {
        ...
        if (this.contextMenu) {
            ...
            document.getElementById('context-connect-nodes-btn').onclick = () => this.handleConnectNodes();
        }
...
/**
    ...
    renderTraceLinkTabs() {
        ...
        let firstProjectVaultId = null; // Corrected variable name
        ...
        if (Object.keys(this.tracelinkProjects).length === 0) {
            ...
            if (firstProjectVaultId) { // Use the correct variable name
    ...
    confirmDeleteProject(vaultId) {
        ...
        if (confirm(`Are you sure you want to delete the TraceLink visualization for "${vaultName}"? This will only delete the visualization data, not the vault or its entries.`)) {
...
/**
        ...
        allVaults.forEach(vault => {
            ...
            vaultItem.innerHTML = `
                ...
                <div class="vault-info">
                    ...
                    <div class="vault-entry-count">${vault.entries.length || 0} entries</div>
                </div>
            `;
    ...
    handleImportFromVaultSubmit(event) {
        ...
        // (Assuming project name is primarily for the *modal*, and the actual project is linked to vaultId)
        // If you want a separate project *name* for TraceLink, you'll need to store it and check
        // For now, TraceLink project is implicitly named after the vault.
        ...
        this.saveTraceLinkProjects();
        this.renderTraceLinkTabs(); // Re-render tabs to show new projects
    ...
    importEntitiesToCurrentGraph() {
        ...
        if (elementsToAdd.length > 0) {
            ...
            this.saveGraphState(); // Save after adding/updating elements
    ...
    showContextMenu(event, type) {
        ...
        // Get the position relative to the Cytoscape container's content area (offset values)
        const menuX = event.originalEvent.offsetX;
        const menuY = event.originalEvent.offsetY;

        this.contextMenu.style.left = `${menuX}px`;
        this.contextMenu.style.top = `${menuY}px`;
        this.contextMenu.style.display = 'block';

        // Adjust position if it goes off screen (simple check based on viewport)
        // Note: This relies on the menu's actual rendered size, which might not be immediate.
        // For more complex adjustments, consider Cytoscape.js's 'renderedPosition' or 'viewport' properties.
    ...
    /**
     * Initiates the node connection mode.
     *
     */
    handleConnectNodes() {
        ...
        if (this._isConnecting) {
             this.showNotification('Already in connection mode. Click target node or right-click canvas to cancel.', 'info', 3000);
             return;
        }

        // Validate contextTarget is a node
        if (!this.contextTarget || !this.contextTarget.isNode()) {
            this.showNotification('Error: No valid source node selected to connect from. Right-click a node to start.', 'error');
            return;
        }

        this._sourceNodeForConnection = this.contextTarget;
        this._sourceNodeForConnection.addClass('connect-source'); // Highlight the source node
        this._isConnecting = true;

        this.showNotification(`Connection mode: Click on a target node to connect to "${this._sourceNodeForConnection.data('label')}". Right-click background to cancel.`, 'info', 5000);

        // One-time listeners for the connection process
        const onNodeClick = (e) => {
            const targetNode = e.target;
            // Ensure target is a node and not the source node itself
            if (targetNode.isNode() && targetNode.id() !== this._sourceNodeForConnection.id()) {
                ...
                if (existingEdge.length === 0) {
                    ...
                    this.showNotification('Nodes are already connected.', 'warning');
        ...
        const onCanvasRightClick = (e) => { // Using cxttap (right click) on canvas to cancel
            if (e.target === this.cy) { // Clicked background
                this.showNotification('Connection cancelled.', 'info');
                this._cancelConnectionMode();
            }
        };

        // Attach listeners for connection mode
        this.cy.on('tap', 'node', onNodeClick); // Single click on any node
        this.cy.on('cxttap', onCanvasRightClick); // Right click on canvas
    }
    ...
    _cancelConnectionMode() {
        if (this._isConnecting) {
            ...
            this.cy.off('tap', 'node'); // Remove specific 'tap' listener for nodes
            this.cy.off('cxttap'); // Remove specific 'cxttap' listener for canvas
            this._isConnecting = false;
            this._sourceNodeForConnection = null;
        }
    }
    ...
    /**
     * Updates the dynamic fields in the add node modal based on the selected entity type.
     *
     */
    ...
    handleAddNodeSubmit(event) {
        ...
        let positionX = (this.cy.width() / 2 - this.cy.pan().x) / this.cy.zoom();
        let positionY = (this.cy.height() / 2 - this.cy.pan().y) / this.cy.zoom();
        
        // If the context click was on the canvas, use that position
        if (this.contextTarget && this.contextTarget === this.cy) {
    ...
    handleContextDelete() {
        ...
        if (this.contextTarget.isNode()) {
            if (confirm(`Are you sure you want to delete node "${this.contextTarget.data('label')}" and its connected edges?`)) {
                this.contextTarget.remove();
                this.saveGraphState();
                this.showNotification('Node and its edges deleted.', 'success');
            }
        } else if (this.contextTarget.isEdge()) {
            if (confirm(`Are you sure you want to delete this edge (${this.contextTarget.data('label') || 'unlabeled'})?`)) {
                this.contextTarget.remove();
                this.saveGraphState();
    ...
    /**
     * Displays a notification message using the global notification system.
     * @param {string} message - The message to display.
    ...
    showNotification(message, type = 'info', duration = 3000) {
        if (typeof window.showNotification === 'function') {
            ...
            console.warn("showNotification function not found. Displaying console log instead:", message);
